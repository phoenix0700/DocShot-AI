...
export const ScreenshotConfigSchema = z.object({
  ...
  waitFor: z.union([
    WaitConditionSchema,
    z.array(WaitConditionSchema)
  ]).optional(),
  authentication: AuthenticationSchema.optional(),
  cookies: z.array(CookieSchema).optional(),
  customCSS: z.string().optional(),
  userAgent: z.string().optional(),
  schedule: z.string().regex(/^(\*|([0-5]?\d)) (\*|([01]?\d|2[0-3])) (\*|([12]?\d|3[01])) (\*|([0]?\d|1[0-2])) (\*|([0-6]))$/).optional(),
...
export const ProjectConfigSchema = z.object({
  version: z.string().regex(/^\d+\.\d+(\.\d+)?$/, 'Version must be in semver format (e.g., "1.0" or "1.0.0")'),
  project: z.object({
    name: z.string().min(1).max(100),
  ...
  screenshots: z.array(ScreenshotConfigSchema).min(1, 'At least one screenshot configuration is required'),
